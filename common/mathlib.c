...
void AngleVectors (vec3_t angles, vec3_t forward, vec3_t right, vec3_t up)
{
	...
	angle = angles[0] * (M_PI * 2.0 / 360.0);
	sp = sin (angle);
	cp = cos (angle);
	angle = angles[1] * (M_PI * 2.0 / 360.0);
	sy = sin (angle);
	cy = cos (angle);

	if (right || up)
	{
		angle = angles[2] * (M_PI * 2.0 / 360.0);
		sr = sin (angle);
		cr = cos (angle);
	}
...
void Vector2Angles (vec3_t in, vec3_t out)
{
	...
	{
		y = atan2 (in[1], in[0]) * 180 / M_PI;
		if (y < 0)
			y += 360;

		p = atan2 (in[2], sqrt(in[0]*in[0]+in[1]*in[1])) * 180 / M_PI;
		if (p < 0)
			p += 360;
	}
