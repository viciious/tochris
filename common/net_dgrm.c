...
static qsocket_t *_Datagram_CheckNewConnections (void)
{
	...
	if (command == CCREQ_SERVER_INFO)
	{
		...
		MSG_WriteByte(&net_message, Com_ServerMaxclients());
	...
	if (command == CCREQ_PLAYER_INFO)
	{
		...
		playerNumber = MSG_ReadByte();
		activeNumber = -1;
		for (clientNumber = 0, client = svs.clients; clientNumber < Com_ServerMaxclients(); clientNumber++, client++)
		{
			if (client->active)
			{
				activeNumber++;
		...
		if (clientNumber == Com_ServerMaxclients())
			return NULL;
