...
{
	qboolean changed;
	extern void SV_BroadcastPrintf (char *fmt, ...);
	...
	if ( (var->value != 0) && (!Q_strcmp (var->name, "deathmatch")) )
		Cvar_Set (Cvar_FindVar("coop"), "0");
	if ( (var->value != 0) && (!Q_strcmp (var->name, "coop")) )
		Cvar_Set (Cvar_FindVar("deathmatch"), "0");
}
...
Cvar_CompleteCountPossible (char *partial)
{
	...
	for (v = cvar_vars; v; v = v->next)
		if (!_strnicmp(partial, v->name, len))
			h++;
