...
#include "winquake.h"
#include "../ref_soft/d_local.h"
#include "resource.h"

#define MAX_MODE_LIST	30
#define VID_ROW_SIZE	3

qboolean	dibonly = true;
...
int lhfindcolor(byte *pal, int colors, int r, int g, int b)
{
...
*/
void VID_MakeColormap8 (void *outcolormap, byte *pal)
{
	int c, l;
	byte *out;
	...
	if (host_colormap)
	{
		memcpy (outcolormap, host_colormap, VID_GRADES*256);
		return;
	}
	...
	for (l = 0;l < VID_GRADES;l++)
	{
		for (c = 0;c < vid.fullbright;c++)
			out[l*256+c] = lhfindcolor(pal, 256, (pal[c*3+0] * l) >> (VID_CBITS - 1), (pal[c*3+1] * l) >> (VID_CBITS - 1), (pal[c*3+2] * l) >> (VID_CBITS - 1));
		for (;c < 256;c++)
			out[l*256+c] = c;
	}
}
...
void VID_MakeColormaps(int fullbrights, byte *pal)
{
	...
	VID_MakeColormap8 (vid.colormap, pal);
}
...
// fullscreen DIBs as well
	...
	vid_testingmode = 0;
	if (host_colormap)
		VID_MakeColormaps(256 - host_colormap[16384], host_basepal);
	else
		VID_MakeColormaps(224, host_basepal);
