...
mleaf_t *Mod_PointInLeaf (vec3_t p, model_t *model)
{
	...
	if (!model || !model->nodes)
		Host_EndGame ("Mod_PointInLeaf: bad model");
...
===============
*/
int Mod_Flags (model_t *model)
{
	if (!model)
		Host_EndGame ("Mod_Flags: bad model");
...
// load the file
//
	buf = (unsigned *)COM_LoadTempFile (mod->name);
	if (!buf)
	{
		if (crash)
...
void Mod_LoadLighting (lump_t *l)
{
	...
	else 
	{
		...
		data = (byte *) COM_LoadTempFile (litfilename);
		...
		if (data)
		{
			if (data[0] == 'Q' && data[1] == 'L' && data[2] == 'I' && data[3] == 'T')
			{
				...
				if (i == 1)
				{
					...
					out = data + 8;
					s = (com_filesize-8) / 3;
					loadmodel->lightdata = Hunk_AllocName (s, litfilename);

					for (i = 0; i < s; i++)
					{
						p = max(out[0], out[1]);
						loadmodel->lightdata[i] = max(p, out[2]);
						out += 3;
					}
...
void Mod_LoadVertexes (lump_t *l)
{
	...
	out = Hunk_AllocName ( (count+8)*sizeof(*out), loadname);		// extra for skybox
...
void Mod_LoadEdges (lump_t *l)
{
	...
	if (l->filelen % sizeof(*in))
		Sys_Error ("MOD_LoadBmodel: funny lump size in %s",loadmodel->name);
	count = l->filelen / sizeof(*in);
	out = Hunk_AllocName ( (count + 13) * sizeof(*out), loadname);	// extra for skybox
...
void Mod_LoadTexinfo (lump_t *l)
{
	...
	if (l->filelen % sizeof(*in))
		Sys_Error ("MOD_LoadBmodel: funny lump size in %s",loadmodel->name);
	count = l->filelen / sizeof(*in);
	out = Hunk_AllocName ( (count+6)*sizeof(*out), loadname);	// extra for skybox
...
{	
	...
	if (l->filelen % sizeof(*in))
		Sys_Error ("MOD_LoadBmodel: funny lump size in %s",loadmodel->name);
	count = l->filelen / sizeof(*in);
	out = Hunk_AllocName ( (count+24)*sizeof(*out), loadname);	// extra for skybox
	...
	trivertx_t *pbboxmin, trivertx_t *pbboxmax, aliashdr_t *pheader, char *name)
{
	...
	// endianness
		...
		if (pbboxmin->v[i] < aliasbboxmin[i])
			aliasbboxmin[i] = pbboxmin->v[i];
		if (pbboxmax->v[i] > aliasbboxmax[i])
			aliasbboxmax[i] = pbboxmax->v[i];
	}
	...
	trivertx_t *pbboxmin, trivertx_t *pbboxmax, aliashdr_t *pheader, char *name)
{
	...
	*pframeindex = (byte *)paliasgroup - (byte *)pheader;
	...
	for (i=0 ; i<numframes ; i++)
	{
		paliasgroup->frames[i].numframes = numframes;
...
//
	...
	for (i = 0; i < 3; i++)
	{
		mod->mins[i] = aliasbboxmin[i] * pmodel->scale[i] + pmodel->scale_origin[i];
		mod->maxs[i] = aliasbboxmax[i] * pmodel->scale[i] + pmodel->scale_origin[i];
	}
