...
void R_Alias_clip_z (finalvert_t *pfv0, finalvert_t *pfv1, finalvert_t *out)
{
	...
	if (out->v[0] < r_oldrefdef.aliasvrect.x)
		out->flags |= ALIAS_LEFT_CLIP;
	if (out->v[1] < r_oldrefdef.aliasvrect.y)
		out->flags |= ALIAS_TOP_CLIP;
	if (out->v[0] > r_oldrefdef.aliasvrectright)
		out->flags |= ALIAS_RIGHT_CLIP;
	if (out->v[1] > r_oldrefdef.aliasvrectbottom)
...
void R_Alias_clip_left (finalvert_t *pfv0, finalvert_t *pfv1, finalvert_t *out)
{
	...
	if (pfv0->v[1] >= pfv1->v[1])
	{
		scale = (float)(r_oldrefdef.aliasvrect.x - pfv0->v[0]) /
				(pfv1->v[0] - pfv0->v[0]);
		for (i=0 ; i<6 ; i++)
	...
	{
		scale = (float)(r_oldrefdef.aliasvrect.x - pfv1->v[0]) /
				(pfv0->v[0] - pfv1->v[0]);
		for (i=0 ; i<6 ; i++)
	...
	finalvert_t *out)
{
	...
	if (pfv0->v[1] >= pfv1->v[1])
	{
		scale = (float)(r_oldrefdef.aliasvrectright - pfv0->v[0]) /
				(pfv1->v[0] - pfv0->v[0]);
		for (i=0 ; i<6 ; i++)
			out->v[i] = pfv0->v[i] + (pfv1->v[i] - pfv0->v[i])*scale + 0.5;
	}
	...
	{
		scale = (float)(r_oldrefdef.aliasvrectright - pfv1->v[0]) /
				(pfv0->v[0] - pfv1->v[0]);
		for (i=0 ; i<6 ; i++)
			out->v[i] = pfv1->v[i] + (pfv0->v[i] - pfv1->v[i])*scale + 0.5;
	}
...
void R_Alias_clip_top (finalvert_t *pfv0, finalvert_t *pfv1, finalvert_t *out)
{
	...
	if (pfv0->v[1] >= pfv1->v[1])
	{
		scale = (float)(r_oldrefdef.aliasvrect.y - pfv0->v[1]) /
				(pfv1->v[1] - pfv0->v[1]);
		for (i=0 ; i<6 ; i++)
			out->v[i] = pfv0->v[i] + (pfv1->v[i] - pfv0->v[i])*scale + 0.5;
	}
	...
	{
		scale = (float)(r_oldrefdef.aliasvrect.y - pfv1->v[1]) /
				(pfv0->v[1] - pfv1->v[1]);
		for (i=0 ; i<6 ; i++)
			out->v[i] = pfv1->v[i] + (pfv0->v[i] - pfv1->v[i])*scale + 0.5;
	}
	...
	finalvert_t *out)
{
	...
	if (pfv0->v[1] >= pfv1->v[1])
	{
		scale = (float)(r_oldrefdef.aliasvrectbottom - pfv0->v[1]) /
				(pfv1->v[1] - pfv0->v[1]);
	...
	{
		scale = (float)(r_oldrefdef.aliasvrectbottom - pfv1->v[1]) /
				(pfv0->v[1] - pfv1->v[1]);
	...
	void(*clip)(finalvert_t *pfv0, finalvert_t *pfv1, finalvert_t *out) )
{
	...
	for (i=0 ; i<count ; j = i, i++)
	{
		...
		if (oldflags ^ flags)
		{
			...
			out[k].flags = 0;
			if (out[k].v[0] < r_oldrefdef.aliasvrect.x)
				out[k].flags |= ALIAS_LEFT_CLIP;
			if (out[k].v[1] < r_oldrefdef.aliasvrect.y)
				out[k].flags |= ALIAS_TOP_CLIP;
			if (out[k].v[0] > r_oldrefdef.aliasvrectright)
				out[k].flags |= ALIAS_RIGHT_CLIP;
			if (out[k].v[1] > r_oldrefdef.aliasvrectbottom)
...
// clip
	...
	for (i=0 ; i<k ; i++)
	{
		if (fv[pingpong][i].v[0] < r_oldrefdef.aliasvrect.x)
			fv[pingpong][i].v[0] = r_oldrefdef.aliasvrect.x;
		else if (fv[pingpong][i].v[0] > r_oldrefdef.aliasvrectright)
			fv[pingpong][i].v[0] = r_oldrefdef.aliasvrectright;

		if (fv[pingpong][i].v[1] < r_oldrefdef.aliasvrect.y)
			fv[pingpong][i].v[1] = r_oldrefdef.aliasvrect.y;
		else if (fv[pingpong][i].v[1] > r_oldrefdef.aliasvrectbottom)
			fv[pingpong][i].v[1] = r_oldrefdef.aliasvrectbottom;
